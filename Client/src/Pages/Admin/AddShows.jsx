          ...
          {Object.entries(dateTimeSelection).map(([date, times]) => (
            <li keys={date}>
              ...
              <div className='flex flex-wrap gap-2 mt-1 text-sm'>
                {times.map((time) => (
                  <div keys={time} className='border border-primary px-2 py-1 flex items-center rounded'>
                    <span>{time}</span>
                    <DeleteIcon onClick={() => handleRemoveTime(date, time)} width={15} className='ml-2 texxt-red-500 hover:text-red-700 cursor-pointer' />
                  </div>
                ))}
              </div>
            </li>
